	...
	<body>
		<div class="grid">
			...
			<div class="grid_item" id="primary_column">
				<?php
					//grabbing the add value from the url -> to diplsay a respected header for the date
					if($_GET['add']!=null){
						echo '<h2>New Note ('.$_GET['add'].'):</h2>';
					}
				?>
			<form method="post">
    			<input name="note" placeholder="Enter note here..." type="text" autofocus><br>
    			<input type="submit" name="submit">
			</form>
    			<?php
    				//if the submit is clicked on the form
    				if(isset($_POST['submit'])){
    					//function -> add to respected date a note/info
    					calendar::add_day_info($_GET['add'],$_POST['note']);
    				}
    			?>
			</div>
